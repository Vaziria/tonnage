...
const char * const BITCOIN_CONF_FILENAME = "tonnage.conf";
const char * const BITCOIN_PID_FILENAME = "tonnaged.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "tonnage" is a composite category enabling all Tonnage-related debug output
            if(ptrCategory->count(string("tonnage"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TonnageCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TonnageCore
    // Mac: ~/Library/Application Support/TonnageCore
    // Unix: ~/.tonnagecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TonnageCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TonnageCore";
#else
    // Unix
    return pathRet / ".tonnagecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty tonnage.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override tonnage.conf
